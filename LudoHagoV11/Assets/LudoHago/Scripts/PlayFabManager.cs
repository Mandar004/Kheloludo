...
public class PlayFabManager : Photon.PunBehaviour
{
    ...
    IEnumerator SendbidAmount()
    {
        Debug.Log("Send Bid Amount Success");
        form = new WWWForm();
        form.AddField("mobile", PlayerPrefs.GetString("mobileno"));
        form.AddField("bidamount", PlayerPrefs.GetString("bidamount"));
    ...
    public void SendPlayerCounttodb()
    {

        if (GameManager.Instance.type == MyGameType.TwoPlayer)
        {
            StartCoroutine(SendPlayerGameCount("TwoPlayer"));
        }
        if (GameManager.Instance.type == MyGameType.FourPlayer)
        {
            StartCoroutine(SendPlayerGameCount("FourPlayer"));
        }
