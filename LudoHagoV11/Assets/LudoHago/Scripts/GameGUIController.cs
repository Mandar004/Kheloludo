...
public class GameGUIController : PunBehaviour
{
    ...
    public GameObject WinCanvascom; 
    ...
    public Setwinnerprice setwinnerprice;
    ...
    void Start()
    {
        ...
        {
            firstPlacePrize = 4 * GameManager.Instance.payoutCoins;
            secondPlacePrize = 0;
        }
        else
        {
        ...
        if (GameManager.Instance.type == MyGameType.Computer) {
            ...
            firstPrizeObject.SetActive(false); }
        else { 
        int firstPlacePrizewithcommission = firstPlacePrize - (firstPlacePrize / 10);
        firstPrizeText.GetComponent<Text>().text = firstPlacePrize + "";
        firstPrizeText.GetComponent<Text>().text = firstPlacePrize + "";
         firstPrizeTextScreenWinScreen.GetComponent<Text>().text = firstPlacePrizewithcommission + "";
        secondPrizeText.GetComponent<Text>().text = secondPlacePrize + "";
            //SendWinningAMount
        PlayerPrefs.SetString("winningamount", firstPlacePrizewithcommission.ToString());
        PlayerPrefs.Save();
        }
        ...
        StartCoroutine(waitForPlayersToStart());
            ...
            int value = PlayerPrefs.GetInt(StaticStrings.SoundsKey);
            if (value==0)
            {
            PlayerPrefs.SetInt(StaticStrings.SoundsKey,1);
            ...
            if (me)
            {
                ...
                if (position == 1)
                {
                    ...
                    if (GameManager.Instance.type == MyGameType.TwoPlayer)
                    {
                        ...
                        StartCoroutine(Sendkwinnergaemtypedata("Twoplayer"));
                        //PlayerPrefs.SetString("playertype", "2");
                        // PlayerPrefs.SetString("prize", firstPlacePrize.ToString());
                        // PlayerPrefs.Save();
                        //StartCoroutine(Sendkwinnergaemtypedata());
                    }
                    else if (GameManager.Instance.type == MyGameType.FourPlayer)
                    {
                        ...
                        StartCoroutine(Sendkwinnergaemtypedata("FourPlayer"));
                        // PlayerPrefs.SetString("playertype", "4");
                        // PlayerPrefs.SetString("prize", firstPlacePrize.ToString());
                        //PlayerPrefs.Save();
                        //StartCoroutine(Sendkwinnergaemtypedata());
                    }
                    ...
                    {
                       // data.Add(MyPlayerData.FourPlayerWinsKey, (GameManager.Instance.myPlayerData.GetFourPlayerWins() + 1).ToString());
                        StartCoroutine(Sendkwinnergaemtypedata("Private"));
                       
                    }
                    if(GameManager.Instance.type == MyGameType.Computer)
                    {
                        WinCanvascom.SetActive(true);
                    }
                    else
                    {
                        GameManager.Instance.myPlayerData.UpdateUserData(data);
                        StartCoroutine(Checkwinner());
                        Debug.Log(PlayerPrefs.GetString("winningamount"));
                ...
                else
                {
                    if (GameManager.Instance.type == MyGameType.TwoPlayer)
                    {
                        StartCoroutine(Sendklosergaemtypedata("TwoPlayer"));
                    }
                    else if (GameManager.Instance.type == MyGameType.FourPlayer)
                    {
                        StartCoroutine(Sendklosergaemtypedata("FourPlayer"));
                    }
                    else if (GameManager.Instance.type == MyGameType.Private)
                    {
                        StartCoroutine(Sendklosergaemtypedata("Private"));
                    }
                    //Dictionary<string, string> data = new Dictionary<string, string>();
    ...
    IEnumerator Checkwinner()
    {
        ...
        WWW w = new WWW(StaticStrings.winnerprizeurl, form);
        yield return w;
    ...
    IEnumerator Sendkwinnergaemtypedata(string gametype)
{
    form = new WWWForm();
    form.AddField("mobile", PlayerPrefs.GetString("mobileno"));
    form.AddField("winningamount", PlayerPrefs.GetString("winningamount"));
    form.AddField("gametype", gametype);
        ...
        WWW w = new WWW(StaticStrings.Sendkwinnergaemtypedata, form);
    yield return w;
    ...
    IEnumerator Sendklosergaemtypedata(string gametype)
    {
        form = new WWWForm();
        form.AddField("mobile", PlayerPrefs.GetString("mobileno"));
        form.AddField("playertype", gametype);
      

        WWW w = new WWW(StaticStrings.Sendkloosergaemtypedata, form);
        yield return w;
    ...
    IEnumerator SesndGameplayedData()
    {
        form = new WWWForm();
        form.AddField("mobile", PlayerPrefs.GetString("mobileno"));
        form.AddField("totalgameplayed", PlayerPrefs.GetInt("totalgameplayed"));

        WWW w = new WWW(StaticStrings.SesndGameplayedData, form);
        yield return w;
